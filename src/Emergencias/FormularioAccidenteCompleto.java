...
import Emergencias.ValidadorAccidente;
...
campoIDPaciente.addFocusListener(new FocusAdapter() {
    ...
    public void focusLost(FocusEvent e) {
        ...
        for (int y = LocalDate.now().getYear(); y <= 2030; y++) comboAnio.addItem(String.valueOf(y));
        for (int h = 0; h < 24; h++) comboHora.addItem(String.format("%02d", h));
        for (int m = 0; m < 60; m++) comboMinuto.addItem(String.format("%02d", m));
